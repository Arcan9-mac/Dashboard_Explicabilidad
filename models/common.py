...
class ConvNextBlock(nn.Module):
    """
    ConvNext Block - VERSIÓN BLINDADA.
    __init__ simplificado para ser compatible con el parser de YOLOv7.
    Los hiperparámetros (k, e) están hardcodeados.
    """
    def __init__(self, c1, c2):  # Acepta solo canales de entrada y salida
        super().__init__()
        # --- Parámetros Hardcodeados ---
        k = 7  # kernel_size
        e = 4  # expansion_factor
        # -----------------------------
        ...
        self.pwconv1 = nn.Linear(c1, c_)
        self.act = nn.GELU()
        self.pwconv2 = nn.Linear(c_, c1)

        # La conexión residual solo es posible si los canales y dimensiones coinciden.
        self.residual = (c1 == c2)

        # La salida debe tener c2 canales.
        self.out_conv = nn.Identity() if self.residual else nn.Conv2d(c1, c2, kernel_size=1)
    ...
    def forward(self, x):
        ...
        if self.residual:
            # Solo sumamos si c1==c2 para evitar errores de canales
            y = input_tensor + y
...
# ... (otras clases como Conv, ConvNextBlock...)

class SwinBlockSPP(nn.Module):
    """
    Reemplazo de SPPCSPC usando los bloques de un stage de Swin Transformer.
    VERSIÓN FINAL CON PADDING: Soluciona el problema de incompatibilidad de tamaño.
    """

    def __init__(self, c1, c2):
        super().__init__()
        # --- Parámetros Hardcodeados ---
        self.window_size = 7  # El tamaño de ventana del modelo que usamos
        # -----------------------------

        full_model = timm.create_model('swin_tiny_patch4_window7_224', pretrained=True)
        self.swin_blocks = full_model.layers[2].blocks

        # Los bloques del Stage 3 (índice 2) esperan 384 canales.
        self.conv_in = Conv(c1, 384, 1, 1)
        self.conv_out = Conv(384, c2, 1, 1)

    def forward(self, x):
        h, w = x.shape[2], x.shape[3]
        x = self.conv_in(x)  # -> (B, 384, H, W)

        # --- INICIO DE LA LÓGICA DE PADDING ---
        # Calculamos el padding necesario para que H y W sean múltiplos de window_size
        pad_h = (self.window_size - h % self.window_size) % self.window_size
        pad_w = (self.window_size - w % self.window_size) % self.window_size

        # Aplicamos el padding a la derecha y abajo
        if pad_h > 0 or pad_w > 0:
            x = F.pad(x, (0, pad_w, 0, pad_h))
        # --- FIN DE LA LÓGICA DE PADDING ---

        # Los bloques esperan (B, H, W, C)
        x = x.permute(0, 2, 3, 1)

        # El tensor ahora tiene un tamaño compatible y no dará error
        x = self.swin_blocks(x)

        # Volvemos a (B, C, H, W)
        x = x.permute(0, 3, 1, 2)

        # --- INICIO DE LA LÓGICA DE RECORTE ---
