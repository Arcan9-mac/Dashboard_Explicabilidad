...
# --- INICIO: LÓGICA DE IMPORTACIÓN ACTUALIZADA ---
try:
    # Solo importamos lo que realmente necesitamos para EigenCAM
...
# --- FIN: LÓGICA DE IMPORTACIÓN ACTUALIZADA ---
...
def generate_feature_maps_images(model, img_rgb, img_tensor, geometry_info, feature_layers_config):
    ...
    all_modules, hooks, features, output_images = list(model.modules()), [], {}, []
...
def generate_grad_cam_image(model, img_rgb, img_tensor, geometry_info, target_layer_identifier):
    """
    Función de visualización de CAM completamente reemplazada con la lógica del notebook.
    Usa SIEMPRE EigenCAM y maneja errores de memoria.
    """
    if not GRAD_CAM_AVAILABLE:
        # Este error ya se maneja en el dashboard, pero es una buena salvaguarda.
        return None
    ...
    try:
        ...
        # Devolvemos un nombre que refleje el método usado
        return (f"EigenCAM (Capa {target_layer_identifier})", cam_image)

    except MemoryError:
        st.warning(f"MemoryError en capa {target_layer_identifier}. Es demasiado grande para analizar con EigenCAM.")
        return None
    except Exception as e:
        st.error(f"Error inesperado al generar CAM para capa {target_layer_identifier}: {e}")
        return None